...
class KPIController {
  ...
  async calculateKPIsIncrementally(measurements) {
    try {
      ...
      // Filtrer les mesures antérieures à la dernière réinitialisation
      const resetTime = this.lastDailyReset || todayStart;
      const filteredMeasurements = measurements.filter(m => new Date(m.id) >= resetTime);
      console.log(`Filtered measurements after reset time (${resetTime.toISOString()}): ${filteredMeasurements.length}/${measurements.length}`);
      ...
      for (let i = 0; i < filteredMeasurements.length - 1; i++) {
        const current = filteredMeasurements[i];
        const next = filteredMeasurements[i + 1];
        ...
        if (current.Luminosité > 800) durations.sunlight += duration;
        else durations.lowLight += duration;
      }
      ...
      // Calcul des moyennes
      const recentMeasurements = [...this.recentMeasurementsCache, ...filteredMeasurements]
      ...
      const kpi = new KPI({
        ...
        lastUpdate: filteredMeasurements.length > 0 ? filteredMeasurements[filteredMeasurements.length - 1].id : prevKPI.lastUpdate,
  ...
  async saveDailyKPIs(kpi) {
    try {
      ...
      const dailyAverages = await this.calculateDailyAverages(new Date(date.getFullYear(), date.getMonth(), date.getDate()));
  ...
  async forceManualReset(type) {
    try {
      ...
      // Traiter toutes les mesures en attente avant réinitialisation
      console.log('Processing remaining buffer before reset:', this.measurementBuffer.length);
      await this.processBuffer();
      ...
      if (type === 'daily') {
        ...
        this.measurementBuffer = [];
        console.log('Daily reset completed: buffer and cache cleared');
      } 
        ...
        this.measurementBuffer = [];
        await this.kpisRef.child('resetTimestamp').set(now.toISOString());
        console.log('Monthly reset completed: buffer and processedTimestamps cleared');
      }
      else if (type === 'full') {
        const kpiSnapshot = await this.kpisRef.child('daily').once('value');
        const kpi = kpiSnapshot.val() ? KPI.fromJSON(kpiSnapshot.val()) : new KPI({});
        ...
        this.processedTimestamps.clear();
        this.measurementBuffer = [];
        console.log('Full reset completed: all KPI fields set to 0, buffer and cache cleared');
      }
      else {
      ...
      console.log('Broadcasted reset KPIs:', resetKPI);
